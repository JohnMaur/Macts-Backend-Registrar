...
//   ctx.body = "Hello World from Railway this is Registrar";
// });
...
app.post('/tagData', async (req, res) => {
  ...
  const timeout = 30000; // 1 minute in milliseconds

  if (tagDataMap[tagData] && now - tagDataMap[tagData] < timeout) {
    console.log("You've already tapped your RFID card. Please wait for 30 seconds before tapping again.");
    io.emit('tagData', { tagData, excessiveTap: true });
    return res.status(429).send({ message: "You've already tapped your RFID card. Please wait for 30 seconds before tapping again." });
  }
